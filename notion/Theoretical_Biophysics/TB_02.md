...
Data: 04/03/2025
Bibliografia: Langevin Equation, FDT theorem, Stores-Einstein relation
...
$$
\begin{equation} m \frac{dv}{dt} = F_{ext}-\zeta v + \delta F \end{equation}
$$

- $F_{ext}$ è una forza deterministica nota
- $\zeta v$  è un termine di dissipazione che rappresenta l'attrito con il fluido circostante
...
### Cosa accade dal punto di vista della fisica statistica?
...
$$
P(S_U) = \frac{\delta(H(S_U)-E)}{\Gamma(E)} \\ S_U=(S_{sistema}, S_{reservoir}) \\ P(S_s)=\sum_k P(S_k, S_r) \\ P(S_s) = \frac{e^{-\beta H}}{z}
$$

Nel caso descritto dall’Equazione $(1)$, invece, l'approccio è diverso: si considera la distribuzione di probabilità a livello **dinamico**. Per tenere conto dell'ambiente, si introducono: il termine di rumore casuale e il termine di dissipazione.
...
$$
\begin{align}
m\frac{d \mathbf{v}}{dt} &= \mathbf{F}_{ext}(\mathbf{x}) - \zeta \mathbf{v} + \delta \mathbf{F}, \\
...
$$
\begin{equation} m\frac{d^2\mathbf{x}}{dt^2} = \mathbf{F}_{ext}(\mathbf{x}) - \zeta \frac{d\mathbf{x}}{dt} + \delta\mathbf{F}(t) \end{equation} 
$$

Questa è una formulazione alternativa, utile per simulazioni numeriche.

### Soluzione dell’equazione di Langevin assenza di forze esterne:
...
Abbiamo osservato che il termine esponenziale agisce come un **memory kernel** per il sistema, pesando il contributo della condizione iniziale e del rumore nel tempo. Ciò significa che gli eventi del passato remoto hanno un peso minore rispetto a quelli del passato recente. 

Il nucleo di memoria ha una sua scala temporale, un tempo caratteristico microscopico determinato dalla relazione:

$$
 \begin{equation} \tau = \frac{m}{\zeta} \end{equation}
$$
...
Questo limite è valido quando si considerano tempi molto più lunghi di $\tau$. Un esempio realistico è il moto browniano: per una particella in un fluido, il tempo caratteristico $\tau$ è dell'ordine di $10^{-12} s$ , mentre i tempi di osservazione sperimentali sono molto più lunghi (millisecondi o secondi). Questo significa che **per il moto browniano si è sempre nel limite overdamped**.
...
$$
\begin{equation} <\delta F_{\alpha}(t)\delta F_{\beta}(t')> = 2B \delta_{\alpha \beta} \delta(t-t') \end{equation}
$$
...
$$
\begin{equation}B = k_b T \; \zeta \end{equation}
$$
...
$$
\begin{equation} D = \lim_{t \to \infty} \frac{\langle \delta x^2 \rangle}{2t}  \end{equation}
$$
...
$$
\begin{equation} \begin{aligned} \langle x_{\alpha} (t)^2 \rangle &= \langle \int_0^t dt' \frac{\delta \bar{F_{\alpha}}(t')}{\zeta}  \int_0^t dt'' \frac{\delta \bar{F_{\alpha}}(t'')}{\zeta} \rangle \\ &= \frac{1}{\zeta^2} \int_0^t dt' \int_0^t dt'' \langle \delta F_{\alpha}(t') \; \delta F_{\alpha}(t'')\rangle \\ &= \frac{1}{\zeta^2} \int_0^t dt' \int_0^t dt'' 2 k_b T \zeta \delta (t'-t'') \\ &= \frac{2 \; k_b \; T \; t}{\zeta} \end{aligned} \end{equation}
$$
...
Usando la relazione $(19)$, si trova:
...
$$
\begin{equation} <\delta F(t_1)\delta F(t_2)> = 2B  \delta(t_1-t_2) \end{equation}
$$

Ora riscaliamo il tempo $t' = \frac{t}{\zeta}$ , e riscriviamo la $(27)$ in funzione di $t’$ : 
...
$$
\begin{align} \tilde{x}(t) &= x(t) = x(\zeta t’) \\ \xi (t')&=\delta F(\zeta t’) \end{align}
$$
...
$$
\begin{equation} <\xi(t'_1) \xi(t'_2)> = <\delta F(\zeta t’_1)\delta F(\zeta t’_2)> = 2 k_b T \zeta   \delta(\zeta(t'_1-t'_2)) \end{equation}
$$

Usiamo la proprietà della delta di Dirac:

$$
\begin{equation} <\xi(t'_1) \xi(t'_2)> = 2 k_b T \zeta   \frac{\delta(t'_1-t'_2)}{\zeta} = 2 k_b T \delta(t'_1-t'_2)\end{equation}
$$
...
Passiamo ora al **processo di Wiener**. La relazione $\frac{d x(t)}{dt}  = \xi(t)$ in matematica, viene scritta come:

$$
\begin{align} dx&=dW \\ W &= \xi (t) dt\end{align}
$$
...
$$
\begin{equation} \begin{aligned} <(dx)^2> &= <(x(t+dt)-x(t))^2> \\ &= \int_t^{t+dt} dt’ \int_t^{t+dt} dt’’ <\xi(t’) \xi(t’’)> = 2 k_b T \delta(t'-t'’) \end{aligned} \end{equation}
$$
