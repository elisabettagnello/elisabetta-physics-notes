...
\begin{tcolorbox}[colback=colorD!10, colframe=colorD!60!colorE, , coltitle=black, title=\textbf{Esito della dimostrazione}]
...
Possiamo mappare il disordine dei legami (o dei siti) in una fluttuazione locale della temperatura critica. Immaginiamo che ogni regione dello spazio $\vec{x}$ abbia una sua temperatura critica locale $T_c(\vec{x})$ che dipende dalla configurazione microscopica del disordine in quel punto.
...
[cite_start]Dove l'esponente $a$ caratterizza il tipo di correlazione del disordine[cite: 3969]:
\begin{itemize}
    \item $a$ grande: disordine a corto raggio (uncorrelated).
    \item $a$ piccolo: disordine a lungo raggio (long-range correlated).
\end{itemize}

Introduciamo la **temperatura ridotta locale** $t(\vec{x})$, normalizzata rispetto alla temperatura critica media $\overline{T_c}$:
\begin{equation}
...
Vogliamo capire se le fluttuazioni locali della temperatura ridotta sono rilevanti su una scala di lunghezza $\xi$. Per farlo, consideriamo un volume di correlazione $V \sim \xi^D$ e calcoliamo la media spaziale della temperatura ridotta in questo volume:
\begin{equation}
t_V = \frac{1}{V} \int_V d^D x \, t(\vec{x})
\end{equation}

Ci interessa confrontare il valor medio di questa quantità con la sua varianza (fluttuazione) dovuta al disordine. [cite_start]Calcoliamo la fluttuazione quadratica media $\Delta^2$ su tutto il sistema (o su diverse realizzazioni del disordine) [cite: 4037-4038]:

\begin{equation}
\Delta^2 = \overline{t_V^2} - \overline{t_V}^2 = \frac{1}{V^2} \int_V d^D x \int_V d^D y \left( \overline{t(\vec{x})t(\vec{y})} - \overline{t(\vec{x})} \, \overline{t(\vec{y})} \right)
\end{equation}

Il termine tra parentesi è proporzionale alla funzione di correlazione $g(\vec{x}-\vec{y})$. Sostituendo l'andamento asintotico $g(r) \sim r^{-a}$:

\begin{equation}
\Delta^2 \propto \frac{1}{V} \int_V d^D r \, g(r) \sim \xi^{-D} \int_0^\xi dr \, r^{D-1} r^{-a} = \xi^{-D} \int_0^\xi dr \, r^{D-1-a}
\end{equation}

[cite_start]L'integrale si comporta diversamente a seconda del valore di $a$ rispetto alla dimensione spaziale $D$ [cite: 4044-4045]:
...
[cite_start]Riassumendo, la fluttuazione del disordine scala come[cite: 3982]:
\begin{equation}
\Delta \sim 
\begin{cases} 
\xi^{-D/2} & \text{se } a > D \\
...
Ora applichiamo il criterio di stabilità. Il disordine è una perturbazione **irrilevante** se, avvicinandosi al punto critico ($\xi \to \infty$), le fluttuazioni del disordine $\Delta$ diventano trascurabili rispetto alla distanza media dal punto critico $t$.
...
\begin{itemize}
    ...
    Richiediamo $a/2 > 1/\nu \implies a\nu > 2$.
    In questo caso, la correlazione a lungo raggio del disordine rende più difficile soddisfare la condizione di stabilità.
\end{itemize}

Se queste disuguaglianze sono violate, il disordine è **rilevante**: il punto fisso puro diventa instabile e il sistema fluisce verso un nuovo punto fisso disordinato con nuovi esponenti critici (o la transizione viene distrutta/smussata).

\section{Criterio di Harris Generalizzato}

Il Criterio di Harris ci permette di capire se il disordine cambia la classe di universalità della transizione (cioè se gli esponenti critici cambiano rispetto al caso puro).
Consideriamo il disordine come una fluttuazione locale della temperatura critica $T_c(\vec{x})$.
Definiamo la funzione di correlazione spaziale delle fluttuazioni di $T_c$:

\begin{equation}
g(\vec{r}) = \overline{T_c(\vec{x}) T_c(\vec{x}+\vec{r})} - \overline{T_c}^2 \sim r^{-a} \quad (\text{per } |\vec{r}| \gg 1)
\end{equation}

Dove l'esponente $a$ caratterizza il tipo di disordine:
\begin{itemize}
    \item $a > D$: Disordine a corto raggio (short-range correlated).
    \item $a < D$: Disordine a lungo raggio (long-range correlated).
\end{itemize}

Valutiamo la varianza delle fluttuazioni della temperatura ridotta locale $t(\vec{x})$ in un volume di correlazione $V \sim \xi^D$:

\begin{equation}
\Delta^2 = \overline{t_V^2} - \overline{t_V}^2 \propto \xi^{-2D} \int_V d^D x \int_V d^D y \, g(\vec{x}-\vec{y})
\end{equation}

L'integrale scala come:

\begin{equation}
\Delta^2 \sim \xi^{-D} \int_0^\xi dr \, r^{D-1} r^{-a} \sim \xi^{-D} \int_0^\xi dr \, r^{D-1-a}
\end{equation}

Il comportamento asintotico dipende dal rapporto tra $a$ e $D$:
\begin{itemize}
    \item \textbf{Se $a > D$ (Convergenza):} L'integrale converge a una costante. La varianza scala come $\Delta^2 \sim \xi^{-D}$. Questo recupera il criterio di Harris standard ($D\nu > 2$).
    \item \textbf{Se $a < D$ (Divergenza):} L'integrale diverge e domina il limite superiore ($\xi^{D-a}$). La varianza scala come:
    \begin{equation}
    \Delta^2 \sim \xi^{-D} \cdot \xi^{D-a} = \xi^{-a}
    \end{equation}
    \item \textbf{Se $a = D$ (Marginale):} Si ha una correzione logaritmica $\Delta^2 \sim \xi^{-D} \log \xi$.
\end{itemize}

Perché il punto fisso puro sia stabile, le fluttuazioni del disordine $\Delta$ devono essere trascurabili rispetto alla distanza dal punto critico $t \sim \xi^{-1/\nu}$ nel limite termodinamico. Se $\Delta \gg t$, il disordine è rilevante e cambia gli esponenti critici (o distrugge la transizione).