...
\noindent La Figura \ref{fig:fixed_points} mostra come (nel caso 3D) il punto fisso puro, situato al limite di assenza di disordine ($p = 1$), sia instabile a causa della rilevanza del disordine ($\alpha > 0$). Le traiettorie del flusso RG si allontanano da esso. Tuttavia, non si dirigono verso il punto fisso di percolazione a $T = 0$ (che è a sua volta instabile lungo la linea critica), ma convergono verso un nuovo punto fisso stabile situato a temperatura finita, detto \textit{Random Fixed Point}. Questo punto fisso controlla il comportamento critico lungo l'intera linea di transizione di fase per $p_c < p < 1$, imponendo una nuova classe di universalità caratterizzata da esponenti critici diversi sia da quelli del modello puro che da quelli della percolazione. 
...
\begin{equation}
H(\vec{S}) = - J \sum_{\langle i,j \rangle} S_i S_j - h \sum_i S_i
\end{equation}
...
\begin{equation}
e^{\beta h S_i} = e^{\beta h} \cdot \begin{cases} z & \text{se } S_i = -1 \\ 1 & \text{se } S_i = +1 \end{cases}
\end{equation}

\noindent Raggruppando le configurazioni in base al numero $n$ di spin orientati verso il basso ($S_i = -1$), l'intera funzione di partizione assume la forma esplicita:
\begin{equation}
\mathcal{Z}(X, z) = e^{\beta h N + \beta J N_B} \sum_{n=0}^N c_n(X) z^n
\end{equation}

\noindent dove $N_B$ è il numero totale di interazioni nel reticolo. I coefficienti $c_n(X)$ sono a loro volta polinomi in $X$ (con esponenti positivi) che tengono conto della configurazione spaziale dei legami. A meno del prefattore esponenziale, $\mathcal{Z}$ diventa quindi un puro polinomio nella variabile complessa $z$ di grado $N$.

\hfill 

\noindent Secondo il teorema di Lee-Yang, le radici della funzione di partizione $\mathcal{Z}$ nel piano complesso della fugacità $z$ godono di due proprietà fondamentali:
\begin{enumerate}
\item Non risiedono mai sull'asse reale positivo. Questo garantisce l'analiticità delle grandezze termodinamiche e l'assenza di transizioni di fase per valori fisici (reali e non nulli) del campo magnetico.
\item Giacciono rigorosamente sulla circonferenza unitaria ($|z|=1$), condizione che corrisponde a un campo magnetico esterno $h$ puramente immaginario.
\end{enumerate}
...
\begin{equation}
f_c = -\frac{1}{\beta |c|} \log \mathcal{Z}_c = \frac{\log y}{2\beta} - \frac{1}{\beta |c|} \sum_{a=1}^{|c|} \log(y - \zeta_a(c))
\end{equation}

\noindent La magnetizzazione del singolo cluster $m_c$ si ottiene derivando l'energia libera rispetto al campo (ricordando che $\frac{\partial}{\partial h} = -2\beta y \frac{\partial}{\partial y}$):
\begin{equation}
...
dove $p=\frac{1}{|\lambda|} \sum_{c} P_{\lambda,c} |c|$ è il numero medio di spin sul reticolo.

\noindent A questo punto, notiamo che cluster diversi possono avere zeri nella stessa posizione. Possiamo raggruppare i termini sommando sugli $N_{\lambda}$ zeri \textit{distinti} possibili nel sistema. Definiamo un "peso" o densità spettrale degli zeri $\eta_a(\lambda)$:

\begin{equation}
m_{\lambda} = p - 2y \sum_{a=1}^{N_{\lambda}} \frac{\eta_a(\lambda)}{y - \zeta_a}
\end{equation}
...
\begin{equation}
\eta_a(\lambda) = \frac{1}{|\lambda|} \sum_{c \, :\, \zeta_a(c) = \zeta_a} \mu_a(c) P_{\lambda,c} > 0
\end{equation}
...
L'espressione finale per $m_{\lambda}$ mostra che \textbf{la magnetizzazione è una somma di poli nel piano complesso della fugacità.}
\begin{itemize}
    \item Se $|y| \neq 1$, i denominatori non si annullano e la funzione è analitica (somma di termini regolari).
    \item I problemi sorgono per $y \to 1$ (limite di campo magnetico esterno nullo, $h \to 0$), dove gli zeri minacciano di far divergere la funzione.
\end{itemize}

\noindent La distanza (il gap) tra lo zero più vicino all'asse reale e il punto critico $y=1$ è inversamente proporzionale alla taglia del cluster. Consideriamo un'isola "pura" (ovvero priva di legami mancanti) molto grande e compatta, costituita da $n \gg 1$ spin: i suoi zeri disteranno dall'asse reale di un fattore $\sim 1/n$. Tuttavia, in un sistema diluito, la probabilità $P_{\lambda,c}$ di trovare una tale fluttuazione crolla in modo esponenziale al crescere della sua dimensione:

\begin{equation}
P_{\lambda,c} \propto p^{|c|} = p^{bn} = e^{-d n}  \quad \quad \text{con } d = b \log(1/p)
\end{equation}

\noindent Nell'avvicinarsi a $h=0$, si innesca quindi una competizione matematica: da un lato il denominatore del polo tende a zero ($\sim 1/n$, spingendo per una divergenza algebrica), dall'altro il numeratore (il residuo $\eta_a$, proporzionale a $P_{\lambda,c}$) tende a zero in modo esponenzialmente veloce ($e^{-dn}$). 

\noindent Poiché lo smorzamento esponenziale domina su qualsiasi divergenza, il contributo di queste fluttuazioni viene violentemente soppresso. La magnetizzazione non diverge e tutte le sue derivate rimangono finite, rendendo la funzione $C^\infty$ (completamente liscia) sull'asse reale. Ciononostante, la presenza di infiniti zeri spinti arbitrariamente vicini all'asse reale distrugge il raggio di convergenza della serie di Taylor. La funzione risulta perciò \textbf{liscia ma non analitica}.


\noindent Queste deboli non-analiticità prendono il nome di \textbf{Singolarità di Griffiths}. Nel concreto, esse rappresentano l'impronta termodinamica di configurazioni spaziali rare ma statisticamente inevitabili nel limite termodinamico. 

Nella \textit{Fase di Griffiths} il reticolo globale è termicamente disordinato (paramagnetico). Tuttavia, queste rare isole giganti si trovano localmente al di sotto della loro temperatura critica. Pur essendo troppo rare e disconnesse per innescare un ordine a lungo raggio (motivo per cui la funzione resta $C^\infty$ e non c'è una vera transizione di fase), esse si comportano localmente come enormi momenti magnetici dotati di un'inerzia colossale. 

Il ruolo delle singolarità di Griffiths è proprio quello di governare la termodinamica e la dinamica in questa regione del diagramma di fase: esse dominano la suscettività a bassi campi e causano un rallentamento anomalo della dinamica di rilassamento del sistema, "ricordando" matematicamente l'esistenza della fase ordinata del modello puro.
