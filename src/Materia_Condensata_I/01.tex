...
\textbf{Docente:} Prof. Sergio Caprara - Stanza 110\\
...
\begin{axis}[
    ...
    xmin=0, xmax=3,
    ymin=-1.5, ymax=1.5,
    xtick={1.122}, xticklabels={$R_0$},
    ytick={-1}, yticklabels={$-|U_0|$},
...
% Approssimazione potenziale di Lennard-Jones
\addplot[colorA, thick] { (1/x)^12 - 2*(1/x)^6 };
\draw[dashed, color=gray] (1.122,0) -- (1.122,-1) -- (0,-1);
\end{axis}
\end{tikzpicture}
\end{center}
...
\section{Reticoli di Bravais: definizione generale}
...
\begin{equazione}{Vettore del reticolo}
\begin{equation}
...
\begin{nota}[Cristalli reali vs Modelli]
...
\section{Reticolo di Bravais in una dimensione}
...
\section{Strutture non-Bravais e concetto di base}
...
\begin{itemize}
    \item Da un punto (es. di colore "rosa"), si vede un vicino a sinistra a 0°, uno a +120° e uno a -120°.
    \item Da un punto adiacente (es. di colore "viola"), si vede un vicino a destra a 0°, uno a +120° a sinistra e uno a -120° a sinistra.
\end{itemize}
...
\subsection{Reticolo monoclino (obliquo)}
\begin{itemize}
    \item $|\mathbf{a}_1| \neq |\mathbf{a}_2|$
    \item Angolo $\varphi \neq 90^\circ$
...
\subsection{Reticolo ortorombico (rettangolare)}
\begin{itemize}
    \item $|\mathbf{a}_1| \neq |\mathbf{a}_2|$
    \item Angolo $\varphi = 90^\circ$
    \item \textbf{Simmetrie}: traslazione, inversione, \textbf{rotazione di 180°} attorno a un asse perpendicolare al piano, \textbf{simmetria speculare} (mirror) rispetto a due piani.
    \item La cella è un rettangolo.
\end{itemize}

\begin{nota}[Distinzione tra monoclino e ortorombico]
\textbf{Distinzione tra monoclino e ortorombico:} la rotazione di 180° è in realtà presente anche nel monoclino in 2D (equivale all'inversione in 2D). Ciò che distingue i due reticoli è la \textbf{simmetria speculare} (mirror symmetry): il rettangolare possiede piani speculari lungo le direzioni principali, mentre il monoclino (obliquo) no. In un reticolo obliquo, la riflessione rispetto alle direzioni principali non mappa il reticolo in sé stesso.
\end{nota}

\subsection{Reticolo esagonale}
\begin{itemize}
...
\subsection{Reticolo tetragonale (quadrato)}
\begin{itemize}
    \item $|\mathbf{a}_1| = |\mathbf{a}_2|$
    \item Angolo $\varphi = 90^\circ$
    \item \textbf{Simmetrie}: gode di \textbf{rotazione di 90°} (che il rettangolare non possiede). Ruotando il reticolo di 90°, esso si sovrappone a sé stesso.
    \item Il reticolo più simmetrico in 2D.
\end{itemize}

\subsection{Reticolo ortorombico centrato}
\begin{itemize}
    \item $|\mathbf{a}_1| = |\mathbf{a}_2|$
...
\begin{itemize}
    \item $a_1 \neq a_2 \neq a_3$ (tutti i lati diversi)
    \item \textbf{Due angoli sono 90°}, un solo angolo è diverso da 90°.
...
\begin{itemize}
    \item $a_1 \neq a_2 \neq a_3$ (tutti i lati diversi)
    \item \textbf{Tutti gli angoli sono 90°}: tutte le facce sono rettangoli (ma tutti diversi tra loro).
...
\begin{itemize}
    ...
    \item \textbf{Tutti gli angoli sono 90°}.
    ...
    \item Il \textbf{tetragonale base centrata} non è un nuovo reticolo: aggiungendo un punto al centro della base quadrata, si ottiene un nuovo reticolo quadrato (ruotato di 45° e più piccolo), che è semplicemente un altro reticolo tetragonale semplice.
...
\begin{itemize}
    ...
    \item \textbf{Tutti gli angoli uguali} in ciascun vertice, ma diversi da 90°.
...
\begin{itemize}
    \item La cella di base è un \textbf{rombo con angolo di 120°}, formato da due triangoli equilateri.
...
\begin{itemize}
    \item $a_1 = a_2 = a_3$, tutti gli angoli = 90°.
    \item Il \textbf{cubo} è l'oggetto \textbf{più simmetrico} tra tutti i sistemi cristallini in 3D.
    \item Possiede moltissime simmetrie. Ad esempio, una \textbf{rotazione di 120°} attorno alla diagonale del corpo mappa il cubo in sé stesso. Esiste un \textbf{esagono nascosto} nel cubo: la sezione del cubo lungo il piano perpendicolare alla diagonale del corpo è un esagono regolare.
...
\begin{nota}[Proprietà Fondamentale]
\textbf{Proprietà fondamentale:} ogni cella primitiva contiene esattamente \textbf{un punto reticolare}.
\end{nota}
...
\begin{itemize}
    \item La cella primitiva del reticolo \textbf{esagonale} è un rombo con angolo di 120°, che non ricorda la simmetria esagonale.
...
\begin{nota}[Costruzione geometrica]
La costruzione della cella di Wigner-Seitz sarà descritta nella prossima lezione.
\end{nota}
...
\begin{itemize}
    \item \textbf{Rotazione di 90°} (simmetria \textit{fourfold}) attorno agli assi passanti per i vertici del poliedro corrispondenti alle facce del cubo.
    \item \textbf{Rotazione di 120°} (simmetria \textit{threefold}) attorno agli assi passanti per i vertici del poliedro corrispondenti alle diagonali del cubo. L'esagono nascosto nel cubo è visibile nella cella di Wigner-Seitz dell'FCC.
\end{itemize}
