...
\begin{equation}
D_2^{8} = \mathcal{F}^{-1} \left\{  \tilde{G}_0(p)^2 \int \frac{d^D q}{(2\pi)^D} \tilde{G}_0(q) \int \frac{d^D k}{(2\pi)^D} \tilde{G}_0(k)^2 \right\}
\end{equation}


\begin{tcolorbox}[colback=colorA!10, colframe=colorB!60!colorA,  title=\textbf{Diagramma a Cactus}]
\begin{equation}
\tilde{D}_2^8(p) =  \left[ \tilde{G}_0(p) \right]^2 \cdot \int \frac{d^D q}{(2\pi)^D} \tilde{G}_0(q) \cdot \int \frac{d^D k}{(2\pi)^D} \tilde{G}_0(k)^2
\end{equation}
\end{tcolorbox}
